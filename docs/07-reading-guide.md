...
> 給「不知道從哪裡開始讀」的自己
> 版本 0.1｜2026-08-15｜對應進度：P0 完成（單元 0.1–0.6）
...
如果你打開程式碼想找「下單怎麼運作」「持倉怎麼算」，會找不到 —— 因為那些還沒寫。P0 這六個單元交付的是**地基**，不是功能：

| 已經有的 | 還沒有的 |
|---|---|
| 金額怎麼表示、怎麼算 | 任何 API 端點（除了健康檢查） |
| 台股規則（跳動單位、手續費、漲跌停） | 下單流程 |
| 資料庫長什麼樣 | 查詢邏輯 |
| 一套假資料 | 前端 |
| 服務能啟動、能連上 DB 與 Redis | 認證 |

所以「看不懂業務邏輯」是正常的 —— **業務邏輯要到 Phase 1（單元 1.1 起）才開始寫。**

現在該理解的是：**這些地基各自在防什麼問題。** 下面每一節都會先講「問題是什麼」，再講「檔案怎麼解決它」。
