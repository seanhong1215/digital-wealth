...
`docs/00-architecture.md` 與 `docs/02-backend.md` 都詳細規範了 schema、索引與交易流程，但**沒有指定資料存取層要用什麼**。單元 0.3（schema）動工前必須決定，因為 0.3–0.6 以及後面整個 Phase 1、Phase 3 都建立在這個選擇上。
...
`PROJECT.md` §2 把「交易一致性」列為第一順位目標。而具體的實作手法是：
...
`PROJECT.md` §8 的學習模式是「每個單元做完即停、導讀、確認理解」。ORM 會把 SQL 藏起來，讀 ORM 的 API 文件學不到「為什麼要鎖這一列」。手寫 SQL 讓每一個決定都是明示的。
...
- 交易一致性、cursor 分頁、行鎖的實作直接可讀，面試時打開檔案就能講
