...
| 疑問 | 影響 | 何時決定 |
|---|---|---|
| 是否支援「市價單」 | MVP 建議只做**限價單**，市價單需要模擬撮合深度 | 單元 3.1 |
| 成交是否允許部分成交 | 建議 MVP **全部成交或全部拒絕**，部分成交會讓狀態機複雜一倍 | 單元 3.1 |
| 是否記錄股利 | 影響 `transactions.type` 列舉。建議記錄，成本低且讓明細更真實 | 單元 0.4 |
...
>
> `PROJECT.md` 原本寫「零股數量另以 decimal 字串處理」，但這對台股是**不必要的**——台股零股交易的最小單位是 **1 股（整數）**，不存在 0.5 股。用 `BIGINT` 就完全足夠。
>
> 需要 `NUMERIC` 的是**美股的碎股（fractional shares）**，可以買 0.137 股 AAPL。若未來要支援美股情境，這個欄位要改型別，而且 `shared/money.ts` 也要跟著加小數運算。
>
> **這個判斷本身是好的面試素材** —— 展示你知道規則差異，而不是無腦套用「金融就要用 decimal」。
...
>
> 這個欄位是很好的面試素材 —— 它展示你想過「資料要怎麼被稽核」，而不只是「資料要怎麼被顯示」。
...
同一個 `seed` + 同一個 `scenario`，每次產生的資料必須完全一致。否則 demo 時重整一次數字就全變了，面試官會困惑。
...
> 資料自洽性是 demo 可信度的關鍵。面試官如果心算發現「持倉成本跟明細對不上」，整個專案的可信度就崩了。
...
>
> **修正（2026-08-15，單元 0.2b）**：本段原寫「股款 108,500 元 + 手續費 154 元 = 總計 108,654 元」，是算術錯誤 —— 上面 request 範例的 `limitPriceCents: 108500`（每股 1085 元）× `quantity: 1000` 股 = **1,085,000 元**，不是 108,500 元。手續費與總計也跟著少一位。正確值已寫成 `shared/src/market-rules.test.ts` 的測試案例。
...
| `RATE_LIMITED` | 429 | 請求過於頻繁 | Toast 提示，禁用送出鈕數秒 |
...
> **為什麼不用 `localStorage`？** 這是新手最常見的做法，也是最常被面試官戳的點。只要頁面上有任何 XSS 漏洞（例如渲染了未逃逸的使用者輸入），攻擊者一行 `localStorage.getItem('token')` 就把身分偷走了。`httpOnly` cookie 從瀏覽器層級阻止 JS 讀取。
>
> **本機部署為什麼還在乎這個？** 因為安全設計是**架構品質的訊號**。面試官看到 `localStorage.setItem('token', ...)` 會直接扣分，看到 httpOnly cookie 會問你「那 CSRF 怎麼辦」—— 後者是你想要的對話。
...
用 Redis 實作滑動視窗，套在寫入端點。

| 端點 | 限制 | 理由 |
|---|---|---|
| `POST /orders` | **10 次 / 分鐘 / 帳戶** | 防止連點與腳本濫用 |
| `POST /auth/login` | **5 次 / 分鐘 / IP** | 防暴力破解 |
| 其他 GET | 120 次 / 分鐘 / IP | 一般保護 |
| WebSocket 訂閱 | 單一連線最多 50 檔標的 | 防止訂閱全市場 |

超過時回 `429 RATE_LIMITED`，並帶 `Retry-After` 標頭。
